...
async def read_company_users(
    ...
    current_user: schema.Users = Depends(auth.get_current_company_admin)
):
    """
    ...
    result = await db.execute(
        select(schema.Users).filter(schema.Users.company_id == current_user.company_id)
    )
...
@router.patch("/employees/{user_id}/approve", response_model=schemas.User)
async def approve_employee_registration(
    user_id: int,
    current_admin: Users = Depends(auth.get_current_company_admin),
    db: AsyncSession = Depends(db_manager.get_db_session)
